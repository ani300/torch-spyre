...
def _insert_one_read_copy(
    ...
    for d in {d for level in sizing_op_info.loop_tiled_dims for d in level}:
        ...
        if d not in squeeze_pos:
            ...
            if sizing_op_info.hint_driven:
                # Use the original input dependency, captured before range
                # division, as the address authority. The output tile's
                # contiguous stride is unrelated for packed expert inputs.
                ...
                # Existing automatic-tiling rule: preserve the squeezed
                # dimension using the reader's canonical output-space stride.
