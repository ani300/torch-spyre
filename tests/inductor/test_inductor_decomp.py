...
"""
    ...
    def test_linear_decomposition_graph(self):
        from torch._dynamo.testing import (
            InductorAndRecordGraphs,
            normalize_gm,
        )
        ...
        torch.compiler.reset()
        backend = InductorAndRecordGraphs()
        cmp = torch.compile(fn_2d, backend=backend)
        x_2d = torch.randn(67, 256, dtype=torch.float16, device="spyre")
        w = torch.randn(128, 256, dtype=torch.float16, device="spyre")
        cmp(x_2d, w)
        ...
        # 3D input: F.linear should decompose via transpose + unsqueeze + bmm
        def fn_3d(x, w):
            return torch.nn.functional.linear(x, w)

        torch.compiler.reset()
        backend = InductorAndRecordGraphs()
        cmp = torch.compile(fn_3d, backend=backend)
        x_3d = torch.randn(3, 17, 256, dtype=torch.float16, device="spyre")
        w = torch.randn(128, 256, dtype=torch.float16, device="spyre")
        cmp(x_3d, w)
    ...
    def test_unflatten_bmm_pass_graph(self):
        from torch._dynamo.testing import (
            InductorAndRecordGraphs,
            normalize_gm,
        )
        ...
        # 3D matmul: view→mm→view should be converted to bmm by _unflatten_mm_to_bmm
        def fn_3d(x, w):
            return x @ w
        ...
        cmp = torch.compile(fn_3d, backend=backend)
        x_3d = torch.randn(3, 17, 256, dtype=torch.float16, device="spyre")
        w_2d = torch.randn(256, 128, dtype=torch.float16, device="spyre")
        cmp(x_3d, w_2d)
        ...
        # 4D matmul: collapsed bmm should be unflattened by unflatten_bmm_batch_dims
        def fn_4d(x, w):
            return x @ w

        torch.compiler.reset()
        backend = InductorAndRecordGraphs()
        cmp = torch.compile(fn_4d, backend=backend)
        x_4d = torch.randn(3, 17, 128, 256, dtype=torch.float16, device="spyre")
        w_4d = torch.randn(3, 17, 256, 128, dtype=torch.float16, device="spyre")
        cmp(x_4d, w_4d)
        ...
        assert "aten.bmm.default" in inductor_graph_str, (
            "Expected aten.bmm.default after unflatten_bmm_batch_dims pass"
        )
